...
// buildStockSummaryText - includes every section only when it has data
{
  const shaped = shapeStockByLocation([
    { location_id: [1, 'Muscat/Stock'], quantity: 40, reserved_quantity: 5 }
  ]);
  const incoming = shapeIncomingStock([]);
  const reserved = shapeReservedTransfers([
    { qty: 3, location: 'Muscat/Stock', pickingName: 'WH/OUT/0001', pickingType: 'Delivery Orders', origin: 'SO/3010' }
  ]);
  const text = buildStockSummaryText(shaped, incoming, reserved, 128);
  assert.ok(text.includes('On Hand: 40.00'));
  assert.ok(text.includes('Stock Value: 128.00 OMR'));
  assert.ok(text.includes('Reserved For:'));
  assert.ok(text.includes('WH/OUT/0001 (Delivery Orders, For SO/3010, Muscat/Stock): 3.00'));
  assert.ok(!text.includes('Incoming:')); // no incoming lines -> section omitted
}

// buildCostSummaryText - foreign price shows the OMR equivalent, landed cost included when known
{
  const costHistory = shapeCostHistory([
    { order_id: 1, price_unit: 8, price_unit_omr: 3.08, currency: 'USD', purchase_type: 'Foreign', product_qty: 30, date_planned: daysAgo(5), order_name: 'PO/0102', vendor: 'Global Parts Co' }
  ]);
  const withLanded = attachLandedCost(costHistory, { 1: { totalValue: 6, entries: [] } });
  const vendors = vendorComparison(withLanded);
  const trend = costTrend(withLanded);
  const text = buildCostSummaryText(withLanded, vendors, 3.2, trend);
  assert.ok(text.includes('Current Avg Cost: 3.20 OMR'));
  assert.ok(text.includes('Last Purchase Price: 8.00 USD (~3.08 OMR) [Foreign]'));
  assert.ok(text.includes('Landed unit cost: 3.08 + 0.20 = 3.28 OMR'));
}

// buildSalesSummaryText - customer and transaction lines both present
{
  const saleLines = [
    { line_id: 1, customer: 'Al Rawahi Trading', customer_id: 501, order_name: 'SO/2001', product_uom_qty: 5, price_unit: 5.5, currency: 'OMR', date_order: daysAgo(5), qty_delivered: 5, qty_invoiced: 5 }
  ];
  const enriched = attachFulfillment(saleLines);
  const shaped = shapeSalesHistory(enriched, { windowDays: 90 });
  const topCustomers = topCustomersForPart(enriched);
  const text = buildSalesSummaryText(shaped, null, topCustomers, enriched);
  assert.ok(text.includes('Total Units Sold: 5.00'));
  assert.ok(text.includes('Top Customers:'));
  assert.ok(text.includes('Al Rawahi Trading: 5.00 units (1 orders, 1 delivered, 1 invoiced)'));
  assert.ok(text.includes('Transactions:'));
  assert.ok(text.includes('SO/2001'));
}
