...
// ---------------- Copy-to-clipboard text builders ----------------
// Plain-text, one function per tab - same convention as
// lib/customer-data.js's buildCustomerSummaryText (simple toFixed(2), no
// dependency on popup.js's locale-aware formatting) so these stay pure and
// testable. Deliberately thorough (every purchase/transaction line, not a
// truncated preview) since a copy action is a deliberate export, unlike the
// paginated on-screen lists.
...
function omrSuffix(currency, amountOmr) {
  return (currency !== 'OMR' && typeof amountOmr === 'number') ? ` (~${fmt(amountOmr)} OMR)` : '';
}

function buildStockSummaryText(shaped, incoming, reserved, stockValue) {
  const lines = [
    'STOCK & LOCATION',
    `On Hand: ${fmt(shaped.totalOnHand)}`,
    `Reserved: ${fmt(shaped.totalReserved)}`
  ];
  if (typeof stockValue === 'number') lines.push(`Stock Value: ${fmt(stockValue)} OMR`);

  if (shaped.byLocation.length) {
    lines.push('By Location:');
    for (const loc of shaped.byLocation) {
      lines.push(`  ${loc.location}: ${fmt(loc.qty)} (Reserved: ${fmt(loc.reserved)})`);
    }
  }
  if (reserved.items.length) {
    lines.push('Reserved For:');
    for (const item of reserved.items) {
      const type = item.pickingType || 'Transfer';
      const origin = item.origin ? `, For ${item.origin}` : '';
      lines.push(`  ${item.pickingName || 'Transfer'} (${type}${origin}, ${item.location || 'Unknown location'}): ${fmt(item.qty)}`);
    }
  }
  if (incoming.items.length) {
    lines.push('Incoming:');
    for (const item of incoming.items) {
      lines.push(`  ${item.orderName || 'Purchase order'} (Expected ${item.datePlanned || 'unknown date'}): ${fmt(item.remaining)}`);
    }
  }
  return lines.join('\n');
}

function buildCostSummaryText(costHistory, vendors, avgCost, trend) {
  const lastPurchase = costHistory.length ? costHistory[0] : null;
  const lines = ['COST', `Current Avg Cost: ${fmt(avgCost)} OMR${trend && trend !== 'insufficient_data' ? ` (${trend})` : ''}`];

  if (lastPurchase) {
    lines.push(
      `Last Purchase Price: ${money(lastPurchase.price, lastPurchase.currency)}` +
      `${omrSuffix(lastPurchase.currency, lastPurchase.priceOmr)} [${lastPurchase.purchaseType || 'Unknown'}]`
    );
  }
  if (vendors.length) {
    lines.push('Vendor Comparison:');
    for (const v of vendors) {
      lines.push(`  ${v.vendor} (${v.purchaseType || 'Unknown'}): ${money(v.price, v.currency)}${omrSuffix(v.currency, v.priceOmr)}`);
    }
  }
  if (costHistory.length) {
    lines.push('Purchase History:');
    for (const c of costHistory) {
      const landed = (typeof c.priceOmr === 'number' && typeof c.landedPerUnit === 'number')
        ? ` - Landed unit cost: ${fmt(c.priceOmr)} + ${fmt(c.landedPerUnit)} = ${fmt(c.priceOmr + c.landedPerUnit)} OMR`
        : '';
      lines.push(
        `  ${c.order || 'Purchase order'} (${c.purchaseType || 'Unknown'}) - ${c.date || 'unknown date'} - ` +
        `${c.vendor || 'Unknown vendor'} - qty ${fmt(c.qty)} - ${money(c.price, c.currency)}${landed}`
      );
    }
  }
  return lines.join('\n');
...
function buildSalesSummaryText(salesShaped, drift, topCustomers, transactions) {
  const lines = [
    'SALES HISTORY',
    `Units Sold (${salesShaped.recentWindowDays}d): ${fmt(salesShaped.recentUnitsSold)}`,
    `Total Units Sold: ${fmt(salesShaped.totalUnitsSold)}`,
    `Last Sold: ${salesShaped.lastSoldDate || 'never'}`
  ];
  if (drift) {
    const sign = drift.diff >= 0 ? '+' : '';
    lines.push(`List Price vs Avg Recent Sale: ${fmt(drift.listPrice)} vs ${fmt(drift.avgRecentPrice)} (${sign}${fmt(drift.diff)})`);
  }
  if (topCustomers.length) {
    lines.push('Top Customers:');
    for (const c of topCustomers) {
      lines.push(`  ${c.customer}: ${fmt(c.qty)} units (${c.orders} orders, ${c.deliveredCount} delivered, ${c.invoicedCount} invoiced)`);
    }
  }
  if (transactions.length) {
    ...
    for (const t of transactions) {
      lines.push(
        `  ${t.order_name || 'Sale order'} - ${t.date_order || 'unknown date'} - ${t.customer} - ` +
        `qty ${fmt(t.product_uom_qty)} - ${money(t.price_unit, t.currency)}${omrSuffix(t.currency, t.price_unit_omr)}`
      );
    }
  }
  return lines.join('\n');
